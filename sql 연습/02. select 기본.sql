...
-- 예제07 : employees 테이블에서 1991년 이전에 입사한 직원의 이름,  성별, 입사일을  입사일 순으로 출력
select first_name as '이름',
gender as '성별',
hire_date as '입사일'
from employees
...
-- in 연산자
-- 예제09 : dept_emp 테이블에서 부서 번호가 d005나 d009에 속한 사원의 사번, 부서번호 출력
...
-- order by
--
-- 예제11 employees 테이블에서 직원의 이름,  성별, 입사일 빠른순으로 출력
select first_name as '이름',
gender as '성별',
hire_date as '입사일'
from employees
...
-- 예제13: 남자 직원의 이름, 성별, 입사일을 선임순(입사일이 작은순)으로 출력하세요.
select first_name, gender, hire_date
from employees
where gender = 'm'
order by hire_date asc;
