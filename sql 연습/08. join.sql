...
-- 예제02) 현재 근무하고 있는 직원의 이름, 직책을 모두 출력하되, 여성 엔지니어만 출력하세요. 
...
-- 1) natural join
-- 조인 대상이 되는 두 테이블에 같은 이름의 컬럼이 있는 경우
-- 조인 조건을 명시 하지 않고 묵시적으로 조인이 됨
...
-- natural join의 문제점
select count(*)
from salaries a natural join titles b -- on a.emp_no = b.emp_no and a.from_date = b.from_date and a.to_date = b.tp_date
...
-- 실습문제 01
-- 현재, 직책별 평균 연봉과 직원 수를 구하되 직원 수가 100명 이상인 직책만 출력 하세요.
-- projection: 직책, 평균 급여, 직원수
